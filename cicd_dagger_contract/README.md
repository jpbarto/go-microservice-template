...
In a typical CI/CD workflow, these functions are invoked in the following stages and order:

### 1. **CI Pipeline** (Continuous Integration)
```
Build → UnitTest
```
- **Build**: Compile and package the application into a container image
- **UnitTest**: Execute unit tests against the built application

### 2. **IAT Pipeline** (Integration Acceptance Testing)
```
Build → UnitTest → IntegrationTest
```
- After passing unit tests, run integration tests against the application

### 3. **Staging Pipeline** (Pre-production Testing)
```
Build → UnitTest → IntegrationTest → Deliver → Deploy → Validate
```
- Full pipeline including delivery to artifact repositories and deployment to staging environment
- May include blue-green deployment testing (deploy current → validate → deploy previous → validate → deploy current)

### 4. **Delivery Pipeline** (Artifact Publication)
```
Build → Deliver
```
- Publish container images and Helm charts to registries after all tests pass

### 5. **Deploy Pipeline** (Production Deployment)
```
Deploy → Validate
```
- Deploy application to production environment and verify deployment health
...
The historical `cicd/local_*.sh` scripts demonstrated common pipeline patterns:

- **`local_ci_pipeline.sh`**: Build → UnitTest
- **`local_iat_pipeline.sh`**: Build → UnitTest → IntegrationTest  
- **`local_staging_pipeline.sh`**: Full pipeline with blue-green testing
- **`local_deliver_pipeline.sh`**: Build → Deliver
- **`local_deploy_pipeline.sh`**: Deploy → Validate
...
3. Chain functions using pipes or variables
